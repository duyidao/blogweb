  ...
  <div ref="pasteRef" class="transition-color">
    <ol>
        <li class="transition-color">
            <p>获取浏览器cookie值</p>
            <code>const cookie = name => `; ${document.cookie}`.split(`; ${name}=`).pop().split(';').shift();
            cookie('_ga');</code>
        </li>
        <li class="transition-color">
            <p>将RGB转换为16进制</p>
        ...
        <li class="transition-color">
            <p>复制到剪切板（使用 navigator.clipboard.writeText 轻松将任何文本复制到剪贴板上）</p>
            <code>const copyToClipboard = (text) => navigator.clipboard.writeText(text);
            copyToClipboard("Hello World");</code>
        </li>
        <li class="transition-color">
            <p>检查日期是否有效</p>
        ...
        <li class="transition-color">
            ...
            // Result: Follow for more</code>
        </li>
        <li class="transition-color">
            <p>计算两个日期之间相差的天数</p>
            <code>const dayDif = (date1, date2) => Math.ceil(Math.abs(date1.getTime() - date2.getTime()) / 86400000)
        ...
        <li class="transition-color">
            <p>清除所有cookie</p>
            <code>const clearCookies = document.cookie.split(';').forEach(cookie => document.cookie = cookie.replace(/^ +/, '').replace(/=.\*/, `=;expires=${new Date(0).toUTCString()};path=/`));</code>
        </li>
        <li class="transition-color">
            <p>生成随机16进制</p>
            <code>const randomHex = () => `#${Math.floor(Math.random() * 0xffffff).toString(16).padEnd(6, "0")}`;
        ...
        <li class="transition-color">
            ...
            // Result: [ 1, 2, 3, 4, 5, 6 ]</code>
        </li>
        <li class="transition-color">
            <p>从URL中获取查询参数</p>
            <code>const getParameters = (URL) => {
                <br/>
        ...
        <li class="transition-color">
            <p>求平值</p>
            <code>const average = (...args) => args.reduce((a, b) => a + b) / args.length;
